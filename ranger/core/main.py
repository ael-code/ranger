    ...
    if not clean:
        ...
        if os.path.exists(custom_comm_path):
            ...
            try:
                ...
                log.debug("Failed to import custom commands from '{}'".format(custom_comm_path))
                log.exception(e)
            else:
                log.debug("Loaded custom commands from '{}'".format(custom_comm_path))
        ...
        try:
            ...
            for plugin in sorted(plugins):
                try:
                    ...
                    log.debug("Loaded plugin '{}'".format(plugin))
                except Exception as e:
                    mex = "Error while loading plugin '{}'".format(plugin)
                    log.error(mex)
                    log.exception(e)
...
def allow_access_to_confdir(confdir, allow):
    ...
    if allow:
        try:
            ...
            log.debug("Created config directory '{}'".format(confdir))
